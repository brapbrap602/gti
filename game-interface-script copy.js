...
document.addEventListener("DOMContentLoaded", function () {
  // Function to check if a line contains 10 or more consecutive dashes/underscores
  function isDashDelimiter(text) {
    const trimmedText = text.trim();
    // UPDATED: Adjusted regex to be more flexible, matching 10+ of various dash/underscore characters.
    // This will catch both the old "---" and new "___" style delimiters.
    const dashPattern = /[-—–_]{10,}/;
    return dashPattern.test(trimmedText);
  }

  // Function to check if an element is a chapter marker
  function isChapterMarker(element) {
    // UPDATED: Changed class check from .calibre2 to .h to match new chapter headings.
    return (
      element &&
  ...
  function addMissingDelimiters() {
    ...
    function processChapterForDelimiters() {
      ...
      if (chapterDelimiters.length % 2 === 1 && chapterDelimiters.length > 0) {
        ...
        if (secondToLastIndex >= 0) {
          ...
          // UPDATED: Changed class from calibre5 to calibre3
          newDelimiter.className = "calibre3"; 
          // UPDATED: Changed delimiter style to match the new format
          newDelimiter.textContent = "_____________"; 
    ...
    for (let i = 0; i < allElements.length; i++) {
      ...
      // Only check p.calibre3 elements for delimiters
      // UPDATED: Changed selector from p.calibre5 to p.calibre3
      if (element.matches && element.matches("p.calibre3")) {
  ...
  function processInterfaceBlocks() {
    ...
    for (let i = 0; i < allElements.length; i++) {
      ...
      // Only process p.calibre3 elements for interface blocks
      // UPDATED: Changed selector from p.calibre5 to p.calibre3
      if (!element.matches("p.calibre3")) {
        continue;
      }
